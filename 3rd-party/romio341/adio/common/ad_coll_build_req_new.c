...
static inline ADIO_Offset view_state_get_next_len(view_state * tmp_view_state_p, int op_type)
{
    flatten_state *tmp_state_p = NULL;
    switch (op_type) {
        case TEMP_OFF:
    ...
    return (ADIO_Offset)
        tmp_view_state_p->flat_type_p->blocklens[tmp_state_p->idx] - tmp_state_p->cur_reg_off;
}
 ...
 * possible later on. */
static inline int view_state_add_region(ADIO_Offset max_sz,
                                        view_state * tmp_view_state_p,
                                        ADIO_Offset * st_reg_p,
                                        ADIO_Offset * tmp_reg_sz_p, int op_type)
{
    ...
    *st_reg_p = tmp_state_p->abs_off;
     ...
     * (i.e. placeholder). */
    assert(tmp_state_p->cur_reg_off != tmp_flat_type_p->blocklens[tmp_state_p->idx]);
    ...
    fprintf(stderr, "view_state:(blocklens[%Ld]=%d,cur_reg_off=%Ld,"
            "max_sz=%Ld)\n", tmp_state_p->idx,
            tmp_flat_type_p->blocklens[tmp_state_p->idx], tmp_state_p->cur_reg_off, max_sz);
#endif

    /* Can it add the whole piece? */
    if (tmp_flat_type_p->blocklens[tmp_state_p->idx] - tmp_state_p->cur_reg_off <= max_sz) {
        data_sz = tmp_flat_type_p->blocklens[tmp_state_p->idx] - tmp_state_p->cur_reg_off;
        ...
        if (tmp_flat_type_p->count == 1) {
            ...
            if (tmp_state_p->idx == (tmp_flat_type_p->count - 1)) {
                tmp_state_p->abs_off += data_sz -
                    ...
                    tmp_flat_type_p->blocklens[tmp_flat_type_p->count - 1] + tmp_view_state_p->ext;
#ifdef DEBUG3
                ...
                tmp_state_p->abs_off +=
                    ...
                    (tmp_flat_type_p->indices[tmp_state_p->idx] + tmp_state_p->cur_reg_off);
#ifdef DEBUG3
                                ...
                                ADIO_Offset * cur_off_p, ADIO_Offset * cur_reg_max_len_p)
{
    ...
    if (tmp_state_p->cur_sz < view_state_p->sz) {
        ...
        ret = get_next_fr_off(fd,
                              tmp_state_p->abs_off,
                              fr_st_off, fr_type_p, &fr_next_off, &fr_max_len);

        while ((tmp_state_p->abs_off < fr_next_off) && (tmp_state_p->cur_sz != view_state_p->sz)) {
            ...
            if (tmp_flat_type_p->count > 1) {
                ...
                if (skip_type_ct > 0) {
                     ...
                     * finished up already */
                    tmp_state_p->cur_sz += skip_type_ct * view_state_p->type_sz;
                    if (tmp_state_p->cur_sz >= view_state_p->sz) {
...
#endif
            view_state_add_region(fr_next_off - tmp_state_p->abs_off,
                                  view_state_p, &tmp_st_off, &tmp_reg_sz, op_type);
                         ...
                         ADIO_Offset * agg_dtype_offset_p, MPI_Datatype * agg_dtype_p)
{
    ...
    if ((client_comm_next_off_arr = (ADIO_Offset *)
        ...
        fprintf(stderr, "ADIOI_Build_agg_reqs: malloc client_next_off_arr " "failed\n");
        return -1;
    }

    if ((client_ol_ct_arr = (int *) ADIOI_Calloc(nprocs, sizeof(int))) == NULL) {
        fprintf(stderr, "ADIOI_Build_agg_reqs: " "malloc client_ol_ct_arr failed\n");
        return -1;
    }
    if ((client_ol_cur_ct_arr = (int *) ADIOI_Calloc(nprocs, sizeof(int))) == NULL) {
        fprintf(stderr, "ADIOI_Build_agg_reqs: " "malloc client_ol_cur_ct_arr failed\n");
        return -1;
    }
    ...
    for (i = 0; i < MAX_OFF_TYPE; i++) {
        ...
        for (j = 0; j < nprocs; j++) {
            find_next_off(fd,
                          ...
                          &(fr_type_arr[agg_idx]), i, &cur_off, &cur_reg_max_len);
        ...
        while (tmp_coll_buf_sz < fd->hints->cb_buffer_size) {
            ...
            fprintf(stderr, "ADIOI_Build_agg_reqs: %s proc %d start/add to"
                    ...
                    cur_reg_max_len, tmp_coll_buf_sz, fd->hints->cb_buffer_size);
#endif
...
#ifdef DEBUG2
                fprintf(stderr, "ADIOI_Build_agg_reqs: " "Warning - Data sieving writes on\n");
#endif
                if (ds_fr_end == -1) {
                    ...
                    fprintf(stderr, "ADIOI_Build_agg_reqs: "
                            ...
                            cur_off, cur_reg_max_len, ds_fr_end);
#endif
                } else {
             ...
             * buffer. */
            if (cur_reg_max_len > (fd->hints->cb_buffer_size - tmp_coll_buf_sz))
                cur_reg_max_len = fd->hints->cb_buffer_size - tmp_coll_buf_sz;

            view_state_add_region(cur_reg_max_len,
            ...
            find_next_off(fd,
                          &client_file_view_state_arr[cur_off_proc],
                          fr_st_off_arr[agg_idx],
                          &(fr_type_arr[agg_idx]), i, &next_off, &next_reg_max_len);
        ...
        if (i == TEMP_OFF) {
            ...
            if ((client_blk_arr = (int **) ADIOI_Malloc(nprocs * sizeof(int *))) == NULL) {
                ADIOI_Free(client_disp_arr);
                fprintf(stderr, "ADIOI_Build_agg_reqs: malloc " "client_blk_arr failed\n");
                return -1;
            }
            for (j = 0; j < nprocs; j++) {
                if ((client_disp_arr[j] =
                     (MPI_Aint *) ADIOI_Malloc(client_ol_ct_arr[j] * sizeof(MPI_Aint))) == NULL) {
                    fprintf(stderr, "ADIOI_Build_agg_reqs: malloc " "client_disp_arr[%d] failed\n",
                            j);
                    return -1;
                }
            ...
            if (agg_ol_ct > 0) {
                if ((agg_disp_arr =
                     (MPI_Aint *) ADIOI_Malloc(agg_ol_ct * sizeof(MPI_Aint))) == NULL) {
    ...
    for (i = 0; i < nprocs; i++) {
        if ((client_file_view_state_arr[i].cur_state.cur_sz !=
             client_file_view_state_arr[i].sz) || client_comm_sz_arr[i] != 0) {
            all_done = 0;
            break;
        }
    }
    ...
    for (i = 0; i < nprocs; i++) {
        if (client_ol_cur_ct_arr[i] != client_ol_ct_arr[i]) {
            fprintf(stderr, "ADIOI_Build_agg_reqs: ERROR Process %d "
                    ...
                    client_ol_cur_ct_arr[i], client_ol_ct_arr[i]);
            return -1;
        }
    }
#ifdef DEBUG1
    ...
    for (i = 0; i < nprocs; i++) {
        fprintf(stderr, "(%d,%d,%Ld)", i, client_ol_ct_arr[i], client_comm_sz_arr[i]);
        if (i != nprocs - 1)
            fprintf(stderr, ",");
    }
    fprintf(stderr, "\n");
    ...
    for (i = 0; i < nprocs; i++) {
        if (client_ol_ct_arr[i] > 0) {
            ...
            for (j = 0; j < client_ol_ct_arr[i]; j++) {
                fprintf(stderr, "[%d](%d,%d) ", j, client_disp_arr[i][j], client_blk_arr[i][j]);
            }
            fprintf(stderr, "\n");
        }
    }
    if (agg_ol_ct) {
        ...
        for (i = 0; i < agg_ol_ct; i++) {
            fprintf(stderr, "[%d](%d,%d)", i, agg_disp_arr[i], agg_blk_arr[i]);
            if (i != agg_ol_ct - 1)
                fprintf(stderr, ",");
        }
        fprintf(stderr, "\n");
                            ...
                            ADIO_Offset * agg_comm_sz_arr, MPI_Datatype * agg_comm_dtype_arr)
{
    ...
    if ((agg_mem_next_off_arr = (ADIO_Offset *) ADIOI_Malloc(nprocs * sizeof(ADIO_Offset))) == NULL) {
        fprintf(stderr, "ADIOI_Build_client_reqs: malloc agg_mem_next_off_arr" "failed\n");
        return -1;
    }

    if ((agg_comm_cur_sz_arr = (ADIO_Offset *)
         ADIOI_Malloc(nprocs * sizeof(ADIO_Offset))) == NULL) {
        fprintf(stderr, "ADIOI_Build_client_reqs: malloc agg_comm_cur_sz_arr" " failed\n");
        return -1;
    }
    if ((agg_ol_ct_arr = (int *) ADIOI_Calloc(nprocs, sizeof(int)))
        == NULL) {
        fprintf(stderr, "ADIOI_Build_client_reqs: " "malloc agg_ol_ct_arr failed\n");
        return -1;
    }
    if ((agg_ol_cur_ct_arr = (int *) ADIOI_Calloc(nprocs, sizeof(int)))
        == NULL) {
        fprintf(stderr, "ADIOI_Build_client_reqs: " "malloc agg_ol_cur_ct_arr failed\n");
        return -1;
    }
    ...
    for (i = 0; i < MAX_OFF_TYPE; i++) {
        ...
        while (total_agg_comm_sz > cur_total_agg_comm_sz) {
            ...
            for (j = 0; j < nprocs; j++) {
                 ...
                 * along. */
                if (tmp_agg_fr_idx < 0 || agg_comm_cur_sz_arr[j] == agg_comm_sz_arr[j]) {
                    continue;
                }
                ...
                find_next_off(fd,
                              ...
                              i, &tmp_cur_off, &tmp_cur_reg_max_len);
                if (tmp_cur_off == -1)
                    continue;

                if ((cur_off == -1) || (cur_off > tmp_cur_off)) {
                    cur_off_proc = j;
                    cur_off = tmp_cur_off;
             ...
             * filetype */
            if (cur_reg_max_len > agg_comm_sz_arr[cur_off_proc] - agg_comm_cur_sz_arr[cur_off_proc]) {
                cur_reg_max_len = agg_comm_sz_arr[cur_off_proc] - agg_comm_cur_sz_arr[cur_off_proc];
            }
            ...
            fprintf(stderr, "ADIOI_Build_client_reqs: %s File region"
                    " (proc=%d,off=%Ld,sz=%Ld)\n",
                    off_type_name[i], cur_off_proc, cur_off, act_reg_sz);
#endif
            ...
            while (view_state_get_cur_sz(tmp_file_state_p, i) - act_reg_sz !=
                ...
                view_state_add_region(view_state_get_cur_sz(tmp_file_state_p, i) - act_reg_sz -
                                      view_state_get_cur_sz(tmp_mem_state_p, i),
                                      tmp_mem_state_p, &fill_st_reg, &fill_reg_sz, i);
            }
            ...
            while (tmp_reg_sz != act_reg_sz) {
                view_state_add_region(act_reg_sz - tmp_reg_sz,
                                      tmp_mem_state_p, &agg_mem_st_reg, &agg_mem_act_reg_sz, i);
                tmp_reg_sz += agg_mem_act_reg_sz;

#ifdef DEBUG2
                fprintf(stderr, "ADIOI_Build_client_reqs: Mem region %s"
                        "(proc=%d,off=%Ld,sz=%Ld)\n",
                        off_type_name[i], cur_off_proc, agg_mem_st_reg, agg_mem_act_reg_sz);
#endif
                ...
                switch (i) {
                    ...
                    case REAL_OFF:
                        ...
                        break;
                    default:
                        fprintf(stderr, "ADIOI_Build_client_reqs: " "Impossible type\n");
                }
            }
        }
        ...
        if (i == TEMP_OFF) {
            ...
            if ((agg_blk_arr = (int **) ADIOI_Malloc(nprocs * sizeof(int *)))
                == NULL) {
                ADIOI_Free(agg_disp_arr);
                fprintf(stderr, "ADIOI_Build_client_reqs: malloc agg_blk_arr failed\n");
                return -1;
            }
            for (j = 0; j < nprocs; j++) {
    ...
    for (i = 0; i < nprocs; i++) {
        int tmp_agg_idx = ADIOI_Agg_idx(i, fd);
        if (tmp_agg_idx >= 0) {
            fprintf(stderr, "(%d,%d=%d)", i, agg_ol_cur_ct_arr[i], agg_ol_ct_arr[i]);
    ...
    for (i = 0; i < nprocs; i++) {
        if (agg_ol_ct_arr[i] > 0) {
            ...
            for (j = 0; j < agg_ol_ct_arr[i]; j++) {
                fprintf(stderr, "[%d](%d,%d) ", j, agg_disp_arr[i][j], agg_blk_arr[i][j]);
            }
            fprintf(stderr, "\n");
        }
    }
                               ...
                               ADIO_Offset max_pre_req_sz, int max_ol_ct)
{
    ...
    if (agg_file_view_state_p->cur_state.cur_sz ==
        agg_file_view_state_p->sz || max_pre_req_sz <= 0 || max_ol_ct <= 0) {
#ifdef DEBUG1
        fprintf(stderr, "ADIOI_Build_client_pre_req: Nothing to preprocess\n");
#endif
        return 0;
    }
...
#ifdef DEBUG1
        fprintf(stderr, "ADIOI_Build_client_pre_req:  Old values surpass new " "pre_req values\n");
#endif
        return 0;
    }
    ...
    for (i = 0; i < MAX_OFF_TYPE; i++) {
        switch (i) {
            ...
            case REAL_OFF:
                ...
                break;
            default:
                fprintf(stderr, "ADIOI_Build_client_pre_req: " "Invalid off type %d\n", i);
        }
        ...
        if (i == TEMP_OFF && my_mem_view_state_p->pre_sz > 0) {
            ...
            agg_mem_next_off = tmp_disp_arr[agg_ol_ct - 1] + tmp_blk_arr[agg_ol_ct - 1];
         ...
         * to preprocess */
        if (max_pre_req_sz - cur_sz > agg_file_view_state_p->sz - tmp_file_state_p->cur_sz) {
            max_sz = cur_sz + agg_file_view_state_p->sz - tmp_file_state_p->cur_sz;
        } else
        ...
        while (cur_sz < max_sz) {
            find_next_off(fd, agg_file_view_state_p,
                          ...
                          &(fr_type_arr[agg_rank]), i, &cur_off, &cur_reg_max_len);
            ...
            while (tmp_file_state_p->cur_sz != tmp_mem_state_p->cur_sz) {
                ...
                if (my_mem_view_state_p->flat_type_p->count > 1) {
                    ...
                    if (skip_type_ct > 0) {
                        tmp_mem_state_p->cur_sz += skip_type_ct * my_mem_view_state_p->type_sz;
                        tmp_mem_state_p->abs_off += skip_type_ct * my_mem_view_state_p->ext;
                        if (tmp_mem_state_p->cur_sz == tmp_file_state_p->cur_sz)
                            break;
                    }
                }
#endif
                view_state_add_region(tmp_file_state_p->cur_sz - tmp_mem_state_p->cur_sz,
                                      my_mem_view_state_p, &fill_st_reg, &fill_reg_sz, i);
            }
            ...
            if (cur_reg_max_len > view_state_get_next_len(agg_file_view_state_p, i))
                cur_reg_max_len = view_state_get_next_len(agg_file_view_state_p, i);
            ...
            act_reg_sz = 0;
            exit_loop = 0;
            while ((act_reg_sz < cur_reg_max_len) && (exit_loop == 0)) {
                view_state_add_region(cur_reg_max_len - act_reg_sz,
                                      my_mem_view_state_p, &agg_mem_st_reg, &agg_mem_act_reg_sz, i);
                act_reg_sz += agg_mem_act_reg_sz;

#ifdef DEBUG2
                fprintf(stderr, "ADIOI_Build_client_pre_req: %s Mem region"
                        "(proc=%d,off=%Ld,sz=%Ld)\n",
                        off_type_name[i], agg_rank, agg_mem_st_reg, agg_mem_act_reg_sz);
#endif
                switch (i) {
                    case TEMP_OFF:
                    ...
                    case REAL_OFF:
                        ...
                        break;
                    default:
                        fprintf(stderr, "ADIOI_Build_client_pre_req: " "Impossible type\n");
                }
            }
            ...
            view_state_add_region(act_reg_sz - tmp_reg_sz,
                                  agg_file_view_state_p, &fill_st_reg, &fill_reg_sz, i);
#ifdef DEBUG2
    ...
    if (agg_ol_ct > 0) {
        ...
        for (i = 0; i < my_mem_view_state_p->pre_ol_ct; i++) {
            fprintf(stderr, "[%d](%d,%d) ", i,
                    my_mem_view_state_p->pre_disp_arr[i], my_mem_view_state_p->pre_blk_arr[i]);
            if (i % 5 == 0 && i != 0)
                fprintf(stderr, "\n");
        }
        fprintf(stderr, "\n");
...
static int process_pre_req(ADIO_File fd,
                           ...
                           int *agg_ol_cur_ct_p, int *agg_ol_ct_p, ADIO_Offset * agg_mem_next_off_p)
{
    int i, has_partial = 0;
    ...
    switch (off_type) {
        case TEMP_OFF:
            ...
            if (my_mem_view_state_p->pre_sz > *agg_comm_sz_p) {
                for (i = 0; i < my_mem_view_state_p->pre_ol_ct; i++) {
                    if ((my_mem_view_state_p->pre_blk_arr[i] + *agg_comm_pre_sz_p) > *agg_comm_sz_p) {
                        has_partial = 1;
                        ...
                        *agg_comm_pre_sz_p = *agg_comm_sz_p;
                        ...
                        break;
                                ...
                                *agg_comm_pre_sz_p) == *agg_comm_sz_p) {
                        *agg_comm_pre_sz_p += my_mem_view_state_p->pre_blk_arr[i];
                        i++;
                        break;
                    } else
                        *agg_comm_pre_sz_p += my_mem_view_state_p->pre_blk_arr[i];
                ...
                if (has_partial == 1) {
                    *agg_mem_next_off_p = my_mem_view_state_p->pre_disp_arr[i - 1] + partial_len;
                } else {
                    *agg_mem_next_off_p =
                ...
                *agg_mem_next_off_p =
                    my_mem_view_state_p->pre_disp_arr[my_mem_view_state_p->pre_ol_ct - 1] +
                    my_mem_view_state_p->pre_blk_arr[my_mem_view_state_p->pre_ol_ct - 1];
            }
#ifdef DEBUG1
        ...
        case REAL_OFF:
            ...
            for (i = 0; i < my_mem_view_state_p->pre_ol_ct; i++) {
                ...
                if ((my_mem_view_state_p->pre_blk_arr[i] +
                    ...
                    break;
                            ...
                            tmp_agg_comm_pre_sz) == *agg_comm_pre_sz_p) {
                    tmp_agg_comm_pre_sz += my_mem_view_state_p->pre_blk_arr[i];
                    i++;
                    break;
                } else
                    tmp_agg_comm_pre_sz += my_mem_view_state_p->pre_blk_arr[i];
            ...
            *agg_comm_cur_sz_p = *agg_comm_pre_sz_p;
            ...
            if ((i < my_mem_view_state_p->pre_ol_ct) || (has_partial == 1)) {
                int remain_ol_ct = my_mem_view_state_p->pre_ol_ct - i + has_partial;
                ...
                if ((new_pre_disp_arr = (MPI_Aint *)
                    ...
                    fprintf(stderr, "process_pre_req: malloc " "new_pre_disp_arr failed\n");
                    return -1;
                }
            ...
            fprintf(stderr, "process_pre_req: REAL_OFF "
                    "agg_comm_pre_sz=%Ld,agg_comm_cur_sz=%Ld,agg_ol_ct=%d,"
                    "agg_ol_cur_ct=%d\n",
                    *agg_comm_pre_sz_p, *agg_comm_cur_sz_p, *agg_ol_ct_p, *agg_ol_cur_ct_p);
#endif
            break;
        default:
    ...
    for (i = 0; i < MAX_OFF_TYPE; i++) {
        ...
        if (my_mem_view_state_p->pre_sz > 0) {
            process_pre_req(fd,
                            ...
                            &agg_comm_pre_sz,
                            &agg_comm_cur_sz,
                            &agg_comm_sz, &agg_ol_cur_ct, &agg_ol_ct, &agg_mem_next_off);
        }
        ...
        while (agg_comm_cur_sz < agg_comm_sz) {
            ...
            view_state_add_region(cur_reg_max_len, agg_file_view_state_p, &st_reg, &act_reg_sz, i);
            ...
            assert(tmp_file_state_p->cur_sz - act_reg_sz >= tmp_mem_state_p->cur_sz);

            while (tmp_file_state_p->cur_sz - act_reg_sz != tmp_mem_state_p->cur_sz) {
                ADIO_Offset fill_st_reg = -1, fill_reg_sz = -1;
#ifdef DTYPE_SKIP
                if (my_mem_view_state_p->flat_type_p->count > 1) {
                    /* let's see if we can skip whole memory datatypes */
                    ...
                    if (skip_type_ct > 0) {
                        tmp_mem_state_p->cur_sz += skip_type_ct * my_mem_view_state_p->type_sz;
                        tmp_mem_state_p->abs_off += skip_type_ct * my_mem_view_state_p->ext;
                        if ((tmp_mem_state_p->cur_sz - act_reg_sz) == tmp_file_state_p->cur_sz)
                            break;
                    }
                }
#endif
                view_state_add_region(tmp_file_state_p->cur_sz -
            ...
            while (tmp_reg_sz != act_reg_sz) {
                ...
                fprintf(stderr, "ADIOI_Build_client_req: %s Mem region"
                        "(off=%Ld,sz=%Ld)\n", off_type_name[i], agg_mem_st_reg, agg_mem_act_reg_sz);
#endif
                ...
                switch (i) {
                    ...
                    case REAL_OFF:
                        ...
                        break;
                    default:
                        fprintf(stderr, "ADIOI_Build_client_req: " "Impossible type\n");
                }
            }
        }
...
#ifdef DEBUG2
    if (my_mem_view_state_p->pre_sz > 0) {
        fprintf(stderr, "ADIOI_Build_client_req: p %d pre(off,len) = ", agg_idx);
        for (i = 0; i < my_mem_view_state_p->pre_ol_ct; i++) {
            fprintf(stderr, "[%d](%d,%d) ", i,
                    my_mem_view_state_p->pre_disp_arr[i], my_mem_view_state_p->pre_blk_arr[i]);
            if (i % 5 == 0 && i != 0)
